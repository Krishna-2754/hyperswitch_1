...
where
    ...
    let auth_header = (
        ...
        format!("Bearer test_KQ3JjvfwvD9J3SHQNCvzc5UfH6WzQq"),
    );
...
impl
    ...
    fn get_request_body(&self, req: &types::PaymentsAuthorizeRouterData) -> CustomResult<Option<String>,errors::ConnectorError> {
        ...
        let mollie_req =
            utils::Encode::<mollie::MolliePaymentsRequest>::convert_and_encode(req).change_context(errors::ConnectorError::RequestEncodingFailed)?;
            println!("<<<<<<<<{:?}", mollie_req);
        Ok(Some(mollie_req))
    }
    ...
    fn handle_response(
        ...
        let response: mollie::MolliePaymentsResponse = res.response.parse_struct("MolliePaymentsResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        logger::debug!(molliepayments_create_response=?response);
        types::ResponseRouterData {
            response,
            data: data.clone(),
...
impl
    ...
    fn get_request_body(&self, req: &types::RefundsRouterData<api::Execute>) -> CustomResult<Option<String>,errors::ConnectorError> {
        let mollie_req = utils::Encode::<mollie::MollieRefundRequest>::convert_and_encode(req).change_context(errors::ConnectorError::RequestEncodingFailed)?;
        Ok(Some(mollie_req))
    }
    ...
    fn handle_response(
        ...
        let response: mollie::RefundResponse = res.response.parse_struct("mollie RefundResponse").change_context(errors::ConnectorError::RequestEncodingFailed)?;
        types::ResponseRouterData {
            response,
            data: data.clone(),
            http_code: res.status_code,
...
impl
    ConnectorIntegration<api::RSync, types::RefundsData, types::RefundsResponseData> for Mollie {
    ...
    fn handle_response(
        ...
        logger::debug!(target: "router::connector::mollie", response=?res);
        let response: mollie::RefundResponse = res.response.parse_struct("mollie RefundResponse").change_context(errors::ConnectorError::ResponseDeserializationFailed)?;
        types::ResponseRouterData {
            response,
            data: data.clone(),
            http_code: res.status_code,
